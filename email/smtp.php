...
function SendMail($frommail,$tomail,$subject,$message,$fromfullname="Booking of DAI TAN RESTAURANT")
{
      ...
      $smtp_host ='mail.nhaduahoian.com';//Dia chi mail server
      $admin_email = 'testmail@nhaduahoian.com';//User duoc khai bao tren mail server
      $smtp_username = 'testmail@nhaduahoian.com';//User duoc khai bao tren mail server
      $smtp_password = 'ABCD#1234abcd';//Pass cua email nay
...
$subject='Booking of web moi';
$mail_to = $_GET["email"];
$name=$_GET["name"];

//SendMail("$mail_to","noreplymilano@hoian30s.com","$subject","$content","$name");
SendMail("$mail_to","test@hoian30s.com","$subject","$content","$name");

if($id==null)
  {
   unset($_SESSION['cart']);
   redirect("thanks.php?lang=$lang&sm2=2#accommo");
  }
  else
  {
  redirect("thanks.php?lang=$lang&sm2=2#accommo");
 // redirect("thanks-order.php?lang=$lang&sm2=2&id=$id");
  }

//smtp_email($mail_to, $subject, $message, $content);
//header("location: contact_us_thankyou.htm");
